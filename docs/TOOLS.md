...
**用法**：
```bash
python tools/search_all.py --config tools/start.json          # 启动后跑（读 slug/领域/关键词）
python tools/search_all.py --slug <slug> --keywords "kw1" "kw2" ...   # 或直接指定
python tools/search_all.py --slug <slug> --parallel 2 --skip-preprints   # 调并行度 / 跳过 P（P2 领域）
```

**并行**：B 查询级并行（`--parallel N` 默认 4，多关键词同时搜、按序落盘，单查询失败不影响其他）+ 通道级并行（B/A/P 三个子进程同时跑）。**退出码**：0=全部通道执行完（零命中属正常，自动登记 empty）；1=某通道崩溃，单独重跑排查。

**web_search.py 多查询**：`--queries-file <json>`（`{"queries": ["q1", ...]}` 或纯列表）+ `--parallel N`（默认 4；1=串行），多组关键词一次跑完、按原顺序落盘 `gathered_web.md` 并登记一次通道 B（done/empty 按总命中数）。
...
**输出**：全部通过退出码 0；检出待确认项退出码 1 并列出位置与命中词。检出项为启发式规则，需人工确认是否真正违规（如"不构成投资建议"中的"建议"为合法用法）。来源特征词含来源/口径/媒体/预算等，带来源括注的列表行与表格行不误报。
...
- 企查查穿透类结果（实控人持股比例、受益股份、财务比率）为服务端精算终值，**逐字引用，禁止自行乘法重算或臆测中间层**（模型多位小数乘法不可靠，已有实际算错案例）。
