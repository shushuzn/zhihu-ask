...
**执行顺序**：**1.1 F → 1.2 E → 1.3 A → 1.4 B → 1.5 C → 1.6 P**，六通道全部执行并登记后过 **1.7 门禁**。领域优先级矩阵（F/B 为通用 P0）：
...
- **2.2 校验**：子问题与视角清单逐一对照，有子问题未被任一视角覆盖即为缺陷（P0 补检索）；补后仍无 → 结论标注"该子问题无公开素材"。直接查询跳过本阶段。
