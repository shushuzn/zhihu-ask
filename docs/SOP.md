...
- **1.1.3 已有笔记复用（必做，不得跳过；过时判定与更新统一在 4.4.3 无条件执行，本阶段不做任何标记）**：
  - ① **拉取全文**：`python tools/flomo_search.py --keywords "<主题词>" --limit 50 --full`（或 `memo_batch_get`）拉取命中笔记全文；
  - ② **忠实还原**：本地 `notes/` 缺失时，按拉回全文**忠实还原本地笔记文件**（仅解除平台转义，不重写内容、不新建主题），flomo 端不动；
...
- **4.4.3 笔记上传/同步（统一动作点；在信息收集与报告成稿后进行，按 .flomo_ids.json 记录分流）**：
  ...
  - **复用笔记（`.flomo_ids.json` 有记录的文件，无条件执行，不依赖任何标记）**：**最终过时核对**——对照阶段 1–3 收集的全部材料逐条复核（来源论文/文章是否有新版本、是否有新结果推翻笔记表述如"尚无定论/待观察/未发布"被证实/证伪）——**确有过时** → 本地改写补结局/改写过时句子后 `python tools/note_upload.py research/<slug>/notes/<文件名>.md --update` 按记录用 `memo_update` 覆盖原 id（禁止新建多版本）；**核对后未过时** → 不动（不执行任何上传/更新命令）。核对结论（逐条：过时/未过时 + 依据）写入 `process_notes.md` 留痕。
...
## A.2 阶段 1 执行逻辑
F 查重最先且阻断：判读 relevance（≥0.9 复用 / 0.5–0.9 参考 / <0.5 正常检索）；≥0.9 时复用必做（1.1.3：拉全文 → 忠实还原本地，本阶段不做任何标记、不执行 memo_update），初步观察提示随 F 通道 note 记录；**过时判定与更新统一在 4.4.3 对全部有记录笔记无条件终核后执行**。E/C 未配置 → 环境级自动 skip；各通道零结果 → 换词重试 1 次 → 记录"无有效素材"；素材落盘自动登记（A/B/P），其余 `mark_channel.py` 登记；`check_progress --require report_channels` 双向校验通过后才进入阶段 2。有效通道 <2 时补充检索，仍不足则告知用户降级。
