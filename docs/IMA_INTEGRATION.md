...
- `openapi/wiki//search_knowledge_base` — 搜索知识库（query/cursor/limit）
- `openapi/wiki//get_addable_knowledge_base_list` — 可添加的知识库列表
- `openapi/note//import_doc` — 新建笔记（content_format + content）
- `openapi/note//search_note_book` / `list_note_by_folder_id` — 检索/列出笔记
...
现状：阶段 0/1 前用 `rag_search.py` 查本地经验（SQLite BM25，词面匹配，索引仅 docs/templates/process_notes）。

增益：ima 为语义检索（RAG），可召回「措辞不同但语义相关」的历史沉淀；且随研究次数增长，ima 中的经验持续累积，形成跨问题「研究大脑」。

### 2.2 沉淀侧 — 公开产物自动入库

现状：每轮研究沉淀到 process_notes.md + SQLite 关键词库（docs/KEYWORDS.md 为导出物），仅本地，跨问题复用靠手动检索。
...
项目红线：research/、plan.md 仅存本地，不推公开仓库（起）。ima 为云知识库，**上云即等同于公开可见性变更，必须逐级确认**：
...
| KEYWORDS.md 增量词条（SQLite 导出物） | 可以 | 与公开仓库一致 |
...
### 方案 B：脚本化（tools/ima_*.py，自动化；**未实施**——待用户提供 OpenAPI 凭证）

基于 OpenAPI 写两个工具，纳入流水线（**以下命令为规划形态，尚未实现**）：
...
前置：用户在 https://ima.qq.com/agent-interface 生成 Client ID + API Key，存 `~/.config/ima/`（或环境变量），**凭证不进项目文件、不入库**（沿用 CLI 类工具凭证安全约定）。
