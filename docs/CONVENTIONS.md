...
- **flomo MCP Token**：`FLOMO_MCP_TOKEN` 写入项目根 `.env`（tools/flomo_search.py 启动时自动读取）。曾硬编码在 flomo_search.py 并进入公开仓库——**须在 flomo 后台撤销旧 token 重建**，新 token 只进 `.env`，绝不写入代码/文档/日志。
