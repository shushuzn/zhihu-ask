...
8. 大小标题禁止用 `*` 与 `#` 标记：笔记内 `##`/`###`/`####` 等 markdown 标题行一律禁止（含 `#`/`*` 任一标记），标题一律用纯文本，不用 `#`/`##`/`###` 层级标记、也不用 `*` 强调代替；仅首行 tag 行允许 `#`（如 `#技术 #AI`），body 内不得出现 `#` 标题行。由 `quality_check.py` 笔记模式 `check_title_asterisk` 与 `check_title_hash` 在 flomo 上传前拦截（命中即阻断上传）。
